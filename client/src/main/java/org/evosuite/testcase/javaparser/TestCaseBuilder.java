...
public class TestCaseBuilder {
	...
	public VariableReference appendConstructor(Constructor<?> constructor, VariableReference... parameters) {
		...
		ConstructorStatement constructorStmt = new ConstructorStatement(testCase,
				new GenericConstructor(constructor, constructor.getDeclaringClass()), parameter_list);
	...
	public VariableReference appendIntPrimitive(int intValue) {
		IntPrimitiveStatement primitiveStmt = new IntPrimitiveStatement(testCase, intValue);
	...
	public VariableReference appendMethod(VariableReference callee, Method method, VariableReference... parameters) {
		...
		if (callee == null) {
			methodStmt = new MethodStatement(testCase, new GenericMethod(method, method.getDeclaringClass()), callee,
					parameter_list);
		} else {
			methodStmt = new MethodStatement(testCase, new GenericMethod(method, callee.getType()), callee,
					parameter_list);
		}
	...
	public VariableReference appendStringPrimitive(String string) {
		StringPrimitiveStatement primitiveStmt = new StringPrimitiveStatement(testCase, string);
		addStatement(primitiveStmt);
		return primitiveStmt.getReturnValue();
	}

	public VariableReference appendBooleanPrimitive(boolean b) {
		BooleanPrimitiveStatement primitiveStmt = new BooleanPrimitiveStatement(testCase, b);
		addStatement(primitiveStmt);
		return primitiveStmt.getReturnValue();
	}

	public VariableReference appendDoublePrimitive(double d) {
		DoublePrimitiveStatement primitiveStmt = new DoublePrimitiveStatement(testCase, d);
		addStatement(primitiveStmt);
		return primitiveStmt.getReturnValue();
	}
	...
	public void appendAssignment(VariableReference receiver, Field field, VariableReference value) {
		...
		if (receiver == null) {
			fieldReference = new FieldReference(testCase, new GenericField(field, field.getDeclaringClass()));
		} else {
			fieldReference = new FieldReference(testCase, new GenericField(field, receiver.getType()), receiver);
		}
		AssignmentStatement stmt = new AssignmentStatement(testCase, fieldReference, value);
		addStatement(stmt);
	}
	...
	public VariableReference appendFieldStmt(VariableReference receiver, Field field) {
		...
		FieldStatement stmt = new FieldStatement(testCase, new GenericField(field, receiver.getType()), receiver);
		addStatement(stmt);
		return stmt.getReturnValue();
	}
	...
	public VariableReference appendEnumPrimitive(Enum<?> value) {
		EnumPrimitiveStatement primitiveStmt = new EnumPrimitiveStatement(testCase, value);
		addStatement(primitiveStmt);
		return primitiveStmt.getReturnValue();
	}
	 ...
	 * @param var
	 */
	public void appendAssignment(ArrayReference array, List<Integer> index, VariableReference var) {
	...
	public VariableReference appendFloatPrimitive(float f) {
		FloatPrimitiveStatement primitiveStmt = new FloatPrimitiveStatement(testCase, f);
		addStatement(primitiveStmt);
		return primitiveStmt.getReturnValue();
	}

	public VariableReference appendShortPrimitive(short s) {
		ShortPrimitiveStatement primitiveStmt = new ShortPrimitiveStatement(testCase, s);
		addStatement(primitiveStmt);
		return primitiveStmt.getReturnValue();
	}
	...
	public void appendAssignment(VariableReference receiver, Field field, VariableReference src, Field fieldSrc) {
		...
		if (receiver == null) {
			dstFieldReference = new FieldReference(testCase, new GenericField(field, field.getDeclaringClass()));
		} else {
			dstFieldReference = new FieldReference(testCase, new GenericField(field, receiver.getType()), receiver);
		}

		FieldReference srcFieldReference;
		if (src == null) {
			srcFieldReference = new FieldReference(testCase, new GenericField(fieldSrc, fieldSrc.getDeclaringClass()));
		} else {
			srcFieldReference = new FieldReference(testCase, new GenericField(fieldSrc, src.getType()), src);
		}
		AssignmentStatement stmt = new AssignmentStatement(testCase, dstFieldReference, srcFieldReference);
