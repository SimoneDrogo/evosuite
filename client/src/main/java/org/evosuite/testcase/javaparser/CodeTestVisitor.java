...
public class CodeTestVisitor {
	...
	private static VariableReference appendPrimitiveStmt(Expression expr, VisitorContext context) {
		...
		if (expr.isStringLiteralExpr()) {
			...
			VariableReference vr = context.getBuilder()
					.appendStringPrimitive(expr.toString().substring(1, expr.toString().length() - 1));
		...
		if (expr.isArrayCreationExpr()) {
			...
			visitor.visit(expr.asArrayCreationExpr(), context);

			return context.getTracker().getReference(expr.asArrayAccessExpr().toString());
		...
		if (expr.isMethodCallExpr()) {
			...
			String methodName = context.getVariableName();

			context.setVariableName("");

			return context.getTracker().getReference(methodName);
	...
	private static VariableReference getReferenceByExpression(FieldAccessExpr fieldAccessExpr, VisitorContext context) {
		...
		if (fieldAccessExpr.resolve().asField().isStatic()) {
			...
			VariableReference vrReciver = context.getTracker().getReference(variableName);
	...
	private static Class<?> classParamByType(ResolvedType paramType) {
		String className = "";
	...
	private static class TestVisitor extends VoidVisitorAdapter<VisitorContext> {
		...
		public void visit(VariableDeclarator vd, VisitorContext context) {
			...
			if (vd.getInitializer().isPresent() && context.getBuilder()!=null) {
				...
				if (expr.isStringLiteralExpr()) {
					...
					VariableReference vr = context.getBuilder()
							.appendStringPrimitive(initializer.substring(1, initializer.length() - 1));
	...
	public static void main(String[] args) throws Exception {
		...
		// 3. Ora puoi passare cu ai tuoi visitor, e il resolve() funzionerà dentro il
		// visitor
